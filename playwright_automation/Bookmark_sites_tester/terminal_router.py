...
# Ensure we can import from backlink_automation modules directly
playwright_automation_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
backlink_automation_dir = os.path.join(playwright_automation_dir, "backlink_automation")
sys.path.append(backlink_automation_dir)
...
# Global browser manager, captcha service, and template runner
...
async def main():
    ...
    parser.add_argument("--type", type=str, help="Site type (e.g., pligg, wordpress_submitpro)")
    ...
    if args.type is None:
        supported = template_runner.get_supported_templates()
        args.type = input(f"Enter site type ({', '.join(supported)}): ").strip()
        if not template_runner.is_supported(args.type):
            print(f"Invalid site type. Supported: {supported}")
            sys.exit(1)

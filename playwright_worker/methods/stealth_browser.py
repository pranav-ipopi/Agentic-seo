...
CDP_PORT = 9222
CDP_LOCK = asyncio.Lock()  # Ensure only one task uses the port at a time for this test
...
def launch_persistent_chrome():
    ...
    base_dst = r"C:\Users\HP\Documents\Agentic_SEO\chrome_profiles\profile_1"
    chrome_path = r"C:\Program Files\Google\Chrome\Application\chrome.exe"
    
    for lock in ["SingletonLock", "SingletonCookie", "SingletonSocket"]:
        lock_path = os.path.join(base_dst, lock)
...
class StealthBrowserManager:
    """
    Manages a connection to a running, persistent Chrome clone via CDP.
    Avoids Playwright/SeleniumBase sterile browser fingerprints.
    """
    def __init__(self):
    ...
    async def start(self, target_url: str = ""):
        print("Acquiring CDP port for Stealth Browser...")
        await asyncio.to_thread(launch_persistent_chrome)
        
        # We use a lock so that multiple concurrent tasks don't crash into each other
        # while using the single profile for testing.
        await CDP_LOCK.acquire()
        print(f"Acquired persistent Chrome on port {CDP_PORT}.")

        endpoint_url = f"http://localhost:{CDP_PORT}"
        ...
        for _ in range(5):
            try:
                ...
                print(f"Playwright connected to real Chrome (port {CDP_PORT}).")
                break
            except Exception:
                await asyncio.sleep(1)
        else:
            CDP_LOCK.release()
            raise RuntimeError(f"Failed to connect to CDP port {CDP_PORT}")

        if target_url:
            print(f"Navigating to {target_url} via CDP...")
            try:
                # Use default context (real profile) to inherit cookies/history
                context = self.browser.contexts[0]
                self.page = await context.new_page()
                await self.page.goto(target_url, timeout=30000)
                # Let it settle and check for CF challenge
                await asyncio.sleep(3)
                await handle_cloudflare_challenge(self.page)
            except Exception as e:
                print(f"Error navigating to target url: {e}")
                
        return self.browser

    async def get_page(self):
        """Returns a page from the persistent default context."""
        if not self.browser:
    ...
    async def close(self):
        ...
        # We just close the page we opened and disconnect Playwright.
        try:
        ...
        try:
            ...
            print(f"Error disconnecting CDP: {e}")
        ...
        print(f"Releasing port {CDP_PORT} back to pool.")
        try:
            CDP_LOCK.release()
        except RuntimeError:
            pass

        print("Stealth Browser connection closed.")

...
*This logic is centralized in `methods/cloudflare.py -> cloudflare_updated()`*.
...
2. **Immediate Challenge Verification:** The script immediately calls `cloudflare_updated(page)`. If a challenge is active, it physically solves it using the Bezier logic.
